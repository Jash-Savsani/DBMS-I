...
SELECT REGION,SUM(SALES_AMOUNT) AS TOTAL_AMT
FROM SALES_DATA
GROUP BY REGION;
--2. Display Average Sales Amount by Product 
SELECT PRODUCT,AVG(SALES_AMOUNT) AS AVG_AMT
FROM SALES_DATA
GROUP BY PRODUCT;
--3. Display Maximum Sales Amount by Year 
SELECT YEAR,MAX(SALES_AMOUNT) AS MAX_AMT
FROM SALES_DATA
GROUP BY YEAR;
--4. Display Minimum Sales Amount by Region and Year 
SELECT REGION,YEAR,MIN(SALES_AMOUNT) AS MIN_AMT
FROM SALES_DATA
...
SELECT REGION,COUNT(DISTINCT PRODUCT) AS PRO 
FROM SALES_DATA
GROUP BY REGION
...
SELECT REGION,PRODUCT, MAX(SALES_AMOUNT) AS TOTAL_SALES
FROM SALES_DATA
where YEAR = '2023'
...
SELECT YEAR,REGION,COUNT(SALES_AMOUNT) AS COUNT
FROM SALES_DATA
GROUP BY REGION,YEAR
ORDER BY COUNT(SALES_AMOUNT);
...
SELECT TOP 1 YEAR,MIN(SALES_AMOUNT) AS LOWEST_SALE
FROM SALES_DATA
GROUP BY YEAR
...
SELECT YEAR,PRODUCT,COUNT(PRODUCT) AS TOTAL_PRODUCT
FROM SALES_DATA
GROUP BY YEAR,PRODUCT
HAVING COUNT(PRODUCT)>1;